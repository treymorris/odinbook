...
function UserIndex() {
  ...
  return (
    <div>
      <Navbar />
      <h1 className="text-light text-center mt-5 mb-5">
        User Profile: {user.username}
      </h1>
      <div className="d-flex">
        <img src={userImage} alt=""></img>
        <div>
          <p className="text-light mt-1 p-1 mb-1 ms-5">
            {user.firstname} {user.lastname}
          </p>
          <p className="text-light p-1 mb-1 ms-5">{user.email}</p>
          <p className="text-light p-1 mb-1 ms-5">
            Birthday:{" "}
      ...
      <button className="btn nav-link ps-0 mt-3 mb-5" onClick={handleClick}>
        Send Friend Request
      </button>
    </div>
  );
